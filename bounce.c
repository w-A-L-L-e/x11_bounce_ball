...
static inline void set_pixel(XImage *img, int x, int y,
                             unsigned char r, unsigned char g, unsigned char b) {
    if (x < 0 || x >= win_width || y < 0 || y >= win_height) return;
    unsigned long pixel = (r << 16) | (g << 8) | b;   // RGB24
    unsigned int *dst = (unsigned int *)(img->data + y * img->bytes_per_line + x * 4);
    *dst = pixel;
}
...
void clear_image(XImage *img, unsigned char r, unsigned char g, unsigned char b) {
    ...
    for (int y = 0; y < win_height; y++) {
        ...
        for (int x = 0; x < win_width; x++) row[x] = pixel;
    }
}
...
// Bresenham circle
void draw_circle(XImage *img, int cx, int cy, int radius,
                 unsigned char r, unsigned char g, unsigned char b) {
    int x = 0;
    int y = radius;
    int d = 3 - 2 * radius;
...
int main() {
    ...
    printf("TIP: you can live resize the window and the ball stays in bounds\n\n");
    ...
    while (running) {
        ...
        cx += vx * dt;
        cy += vy * dt;
        if (cx - radius < 0) { cx = radius; vx = -vx; }
        if (cx + radius >= win_width) { cx = win_width - radius - 1; vx = -vx; }
        if (cy - radius < 0) { cy = radius; vy = -vy; }
        if (cy + radius >= win_height) { cy = win_height - radius - 1; vy = -vy; }

        // Draw frame
        clear_image(img, 25, 25, 25); 

        if(random_background) random_image(img);
